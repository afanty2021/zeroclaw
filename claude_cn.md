...
- `docs/` — 面向任务的文档系统（hubs、统一 TOC、参考、操作、安全提案、多语言指南）
- `.github/` — CI、模板、自动化工作流

## 4.1 文档系统契约（必需）

将文档视为一等产品表面，而非合并后的事后补充。

规范入口点：

- 仓库落地页 + 本地化 hubs：`README.md`、`docs/i18n/zh-CN/README.md`、`docs/i18n/ja/README.md`、`docs/i18n/ru/README.md`、`docs/i18n/fr/README.md`、`docs/i18n/vi/README.md`、`docs/i18n/el/README.md`
...
- 统一 TOC：`docs/SUMMARY.md`
- i18n 治理文档：`docs/i18n-guide.md`、`docs/i18n/README.md`、`docs/i18n-coverage.md`
...
- 保持 README/hub 顶级导航和快速路由直观且不重复。
- 在更改导航架构时，保持所有支持的语言环境（`en`、`zh-CN`、`ja`、`ru`、`fr`、`vi`、`el`）的入口点对等。
- 如果变更涉及文档 IA、运行时契约参考或共享文档中的面向用户措辞，在同一 PR 中对当前支持的语言环境执行 i18n 后续处理：
  - 更新语言环境导航链接（`README*`、`docs/README*`、`docs/SUMMARY.md`）。
  - 为每个支持的语言环境更新 `docs/i18n/<locale>/` 下的规范语言环境 hubs 和 summaries。
  - 在等效存在的情况下更新本地化的运行时契约文档（目前 `vi` 和 `el` 有完整树；不要退化 `zh-CN`/`ja`/`ru`/`fr` hub 对等性）。
  - 如果存在 `docs/*.<locale>.md` 兼容性 shim，保持其对齐。
- 当文档导航或共享措辞变更时，遵循 `docs/i18n-guide.md` 作为强制性完成检查清单。
- 保持提案/路线图文档明确标记；避免将提案文本混合到运行时契约文档中。
- 保持项目快照带日期戳，并在被较新日期取代后保持不可变。

### 4.2 文档 i18n 完成门控（必需）

对于任何更改文档 IA、语言环境导航或共享文档措辞的 PR：

1. 使用 `docs/i18n-guide.md` 在同一 PR 中完成 i18n 后续处理。
2. 保持所有支持的语言环境 hubs/summaries可通过规范 `docs/i18n/<locale>/` 路径导航。
3. 当覆盖率状态或语言环境拓扑变更时，更新 `docs/i18n-coverage.md`。
4. 如果任何翻译必须推迟，在 PR 描述中记录明确的所有者 + 后续问题/PR。
...
5. **记录影响**
    ...
    - 如果 CLI/配置/提供商/渠道行为变更，更新相应的运行时契约参考。
    - 如果文档入口点变更，保持所有支持的语言环境 README/docs-hub 导航对齐（`en`、`zh-CN`、`ja`、`ru`、`fr`、`vi`、`el`）。
    - 运行 `docs/i18n-guide.md` 并在 PR 摘要中记录任何明确的 i18n 推迟。
6. **尊重队列卫生**
...
- 默认情况下打开 PR 到 `main`（`dev` 可选用于集成批处理）；不要直接推送到 `dev` 或 `main`。
- `main` 在必需检查和审查策略通过后接受直接 PR 合并。
- 等待必需检查和审查结果后再合并。
- 通过 PR 控制合并（根据仓库策略允许使用 squash/rebase/merge）。
- 合并/关闭后，清理不再需要的任务分支/worktrees。
- 仅在有意维护且拥有明确所有者和目的时保持长期分支。

### 6.1A PR 处置和工作流权威（必需）

- 按以下顺序从仓库本地权威决定合并/关闭结果：`.github/workflows/**`、GitHub 分支保护/rulesets、`docs/pr-workflow.md`，然后是此 `CLAUDE.md`。
- 外部代理技能/模板只是执行辅助；它们不得覆盖仓库本地策略。
- 当必需检查和审查策略满足时，针对 `main` 的普通贡献者 PR 在 main-first 流程下有效；仅用于显式集成批处理时使用 `dev`。
- 当存在高置信度完整性风险信号时，直接关闭 PR（不要取代/重放）：
  - 未批准或无关的仓库品牌重塑尝试（例如替换项目 logo/身份资产）
  - 未授权的平台表面扩展（例如引入 `web` 应用、仪表板、前端堆栈或维护者未请求的 UI 表面）
  - 标题/范围欺骗，隐藏高风险代码更改（例如 `docs:` 标题带有广泛的 `src/**` 更改）
  - 垃圾邮件式或故意有害的负载模式
  - 多域脏捆绑更改，没有安全、可审计的隔离路径
- 如果在审查/实施期间检测到未授权的平台表面扩展，立即向维护者报告并暂停进一步执行，直到给出明确指示。
- 仅当维护者明确想要保留有效工作和归属时使用取代流程。
- 在公共 PR 关闭/阻止评论中，仅陈述直接可操作的原因；不要包括内部决策过程叙述或"非理由"限定词。

### 6.1B 受理人优先门控（必需）

- 对于选择进行主动处理的任何 GitHub issue 或 PR，第一个操作是确保 `@chumyin` 是受理人。
- 这是累加所有权：保持现有受理人，如果缺失则添加 `@chumyin`。
- 在确认受理人分配之前，不要开始分类/审查/实施/合并工作。
- 队列安全规则：仅分配当前活动目标；不要预先分配未来排队的目标。

### 6.2 Worktree 工作流程（所有任务流必需）

使用 Git worktree 安全且可预测地隔离每个活动任务流：

- 每个活动分支/PR 流使用一个专用 worktree；不要在共享默认工作区中实施。
- 保持每个 worktree 在单个分支和单个关注点上；不要在一个 worktree 中混合不相关的编辑。
- 在每次提交/推送之前，在该 worktree 中验证提交卫生（`git status --short` 和 `git diff --cached`），以便仅包括范围文件。
- 在提交/PR 之前在相应的 worktree 中运行验证命令。
- 按范围清晰命名 worktree（例如：`wt/ci-hardening`、`wt/provider-fix`）。
- 在 PR 合并/关闭（或任务放弃）后，删除过时的 worktrees/分支并修剪引用（`git worktree prune`、`git fetch --prune`）。
- 本地 Codex 自动化可能使用单命令清理辅助：`~/.codex/skills/zeroclaw-pr-issue-automation/scripts/cleanup_track.sh --repo-dir <repo_dir> --worktree <worktree_path> --branch <branch_name>`。
- 第 6.1 节的 PR 检查点规则仍然适用于基于 worktree 的开发。
...
### 7.6 文档系统 / README / IA 更改

- 将文档导航视为产品 UX：保留从 README -> docs hub -> SUMMARY -> 类别索引的清晰路径。
- 保持顶级导航简洁；避免在相邻导航块中出现重复链接。
- 当运行时表面变更时，更新相关参考（`commands/providers/channels/config/runbook/troubleshooting`）。
- 当导航或关键措辞变更时，为所有支持的语言环境（`en`、`zh-CN`、`ja`、`ru`、`fr`、`vi`、`el`）保持多语言入口点对等。
- 当共享文档措辞变更时，在同一 PR 中同步支持语言环境的相应本地化文档（或明确记录推迟和后续 PR）。
- 将 `docs/i18n/<locale>/**` 视为本地化 hubs/summaries 的规范；编辑时保持 docs-root 兼容性 shim 对齐。
- 在合并前应用 `docs/i18n-guide.md` 完成检查清单，并在 PR 说明中包括 i18n 状态。
- 对于文档快照，为新冲刺添加新的带日期戳的文件，而非重写历史上下文。
...
- **仅文档/模板**：
    ...
    - 如果涉及 README/docs-hub/SUMMARY/集合索引，验证 EN/ZH-CN/JA/RU/FR/VI/EL 导航对等性
...
- 对于 issue 驱动的工作，在 **PR 正文**中为每个已解决的 issue 添加明确的 issue 关闭关键字（例如 `Closes #1502`）。
- 不要仅依赖 issue 评论来获得链接可见性；评论是补充，而非 PR 正文关闭引用的替代品。
- 默认为每个 issue 一个干净的提交/PR 轨道。对于多个 issue，除非有明确的技术耦合，否则拆分为单独的干净提交/PR。
- 如果多个 issue 在一个 PR 中有意捆绑，在 PR 摘要中明确记录耦合理由。
- 提交卫生是强制性的：仅暂存任务范围的文件，并将不相关的更改拆分为单独的提交/worktrees。
- 完成卫生是强制性的：在合并/关闭后，在开始下一个轨道之前清理过时的本地分支/worktrees。
- 使用约定式提交标题。
...
- 不要尝试仓库品牌重塑/身份替换，除非维护者在当前范围内明确请求。
- 不要引入新的平台表面（例如 `web` 应用、仪表板、前端堆栈或 UI 门户），除非维护者在当前范围内明确请求。
...
## 12) 快速编码护栏（Vibe Coding Guardrails）
